...
import Pyro5.api
from clsLab3lamportMutualExclusion import clsLab3LamportMutualExclusion
...
def start_ProcessMutexMutualExclusionClient():
    try:
       ...
       # remote_process = Pyro5.api.Proxy(uri)
        remote_process = clsLab3LamportMutualExclusion(uri)
        print ("clsLab3lamportMutualExclusion:requestCriticalSection has started for Process")

        remote_process.requestCriticalSection()
        print ("clsLab3lamportMutualExclusion:requestCriticalSection has completed for Process")
        # Simulate critical section work
        print ("clsLab3lamportMutualExclusion:releaseCriticalSection has started for Process")
        
        remote_process.releaseCriticalSection()
        print ("clsLab3lamportMutualExclusion:releaseCriticalSection has completed for Process ")

        """
            # remote_process = Pyro5.api.Proxy(uri)
            remote_processP1 = clsLab3LamportMutualExclusionP(uri)
